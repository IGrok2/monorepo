...
use awc::http::header::{CACHE_CONTROL, HeaderMap};
use bytes::Bytes;
...
use tokio::task;
use crate::handler::pipeline::rules::models::Trustbusting::Cache;

lazy_static!(
    static ref CACHE_INFOS_DB: DashMap<String, bool> = DashMap::new();
);

pub async fn cache_handler(ctx: &RequestContext, resp: &mut ClientResponse, data: &[PipelineData]) -> Option<bool> { // return type not used, but used to skip the program & debugging
    ...
    for i in data {
        if let Some(t) = i.cache_level {
            if t == CacheLevel::Null {
                ...
                level = t;
                match t { // derive the ttl from the cachelevel
                    CacheLevel::Standard(t) | CacheLevel::Aggressive(t) | CacheLevel::IgnoreQueryString(t) => {
    ...
    if ttl.is_none() { // if there's still no time to live after checking the headers, rely on the default
        // here we don't want to override and instead respect headers, but if we can't then we have to do this
        match ctx.domain.caching_settings.default_cache_level { // derive the ttl from the cachelevel
            CacheLevel::Standard(t) | CacheLevel::Aggressive(t) | CacheLevel::IgnoreQueryString(t) => {
                ttl = t;
            }
            _ => {} // don't do anything
    ...
    match ctx.domain.caching_settings.cached_entries.get(&key) {
        Some(t) => {
            if t.level == level { // worried about expired entries? worry not, that shouldn't be done here anyways
    ...
    let body = match resp.body().limit(ctx.domain.internal_settings.cache_file_max).await {
        Ok(t) => t,
        Err(_) => return None
    };

    let val = ctx.domain.caching_settings.cached_entries.insert(key.clone(), Arc::new(CachedObject {
    ...
    ctx.domain.analytic.cached_data.inc_by((body.len() / 1000) as u32);
...
fn check_cache_headers(headers: &HeaderMap) -> Option<((bool, Option<u64>), bool)> { // should be cached, the ttl, and if it should be transformed
    let mut resp = false;
    ...
    match headers.get(CACHE_CONTROL) {
        Some(t) => {
            ...
            if val.contains("public") { // public means that we can just serve the cache with a max-age
                if val.contains("max-age=") {
                    ttl = val.split(", ")
                        .filter(|val| val.starts_with("max-age=")) // get the actual value
                        .next()
                        .map(|max_age| u64::from_str(max_age.trim_start_matches("max-age=")).unwrap_or(0));
                }
            }
