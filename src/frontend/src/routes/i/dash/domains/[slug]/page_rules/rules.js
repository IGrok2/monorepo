const newPageRule = async (rule, domain) => {
  if (new_rule_matches.length === 0) {
    await error_notification(
      "Couldn't stage new page rule",
      "The first rule wasn't filled out",
    );
    return;
  }
  ...
  for (let i = 0; i < new_rule_matches.length; i++) {
    if (
      new_rule_matches[i].triggerSelected === null ||
      new_rule_matches[i].match_type === null
    ) {
      await error_notification(
        "Couldn't stage new page rule",
        `Match ${i + 1} wasn't filled out`,
      );
      return;
    }
    ...
    if (new_rule_matches[i].triggerSelected === "Query string") {
      ...
      if (!reg.test(new_rule_matches[i].trigger.ip)) {
        await error_notification(
          "Couldn't stage new page rule",
          `Match ${i + 1} had an invalid IP address`,
        );
        return;
      }
      ...
      if (
        !new_rule_matches[i].trigger.path ||
        !new_rule_matches[i].trigger.path.includes("/")
      ) {
        await error_notification(
          "Couldn't stage new page rule",
          `Match ${i + 1} didn't have a correct path`,
        );
        return;
      }
      ...
      if (
        !new_rule_matches[i].trigger.asn ||
        isNaN(new_rule_matches[i].trigger.asn)
      ) {
        await error_notification(
          "Couldn't stage new page rule",
          `Match ${i + 1} didn't have a correct ASN. Should be a number!`,
        );
        return;
      }
      ...
      if (
        !new_rule_matches[i].trigger.country ||
        new_rule_matches[i].trigger.country.length !== 2
      ) {
        await error_notification(
          "Couldn't stage new page rule",
          `Match ${i + 1} didn't have a correct country`,
        );
        return;
      }
      ...
      // make sure it's two characters
      if (
        !new_rule_matches[i].trigger.continent ||
        new_rule_matches[i].trigger.continent.length !== 2
      ) {
        await error_notification(
          "Couldn't stage new page rule",
          `Match ${i + 1} didn't have a correct continent`,
        );
        return;
      }
    }
  ...
  if (!new_rule_actiontype) {
    await error_notification(
      "Couldn't stage new page rule",
      `No action type was selected`,
    );
    return;
  } else if (new_rule_actiontype === "Monopoly" && !new_rule_monopoly) {
    await error_notification(
      "Couldn't stage new page rule",
      `Monopoly action was indicated, but no monopoly was selected`,
    );
    return;
  } else if (
    new_rule_actiontype === "Trustbust" &&
    (!new_rule_trustbust || new_rule_trustbust.length === 0)
  ) {
    await error_notification(
      "Couldn't stage new page rule",
      `Trustbust action was indicated, but no trustbust was selected`,
    );
    return;
  }
...
const deletePageRule = async (rule, domain) => {
  await confirmChange(
    `Delete ${rule.new_staged ? `staged` : `existing`} page rule?`,
    `${rule.new_staged ? "Because this rule was staged, nothing will change in your production environment" : "This rule will be staged for deletion"}`,
    () => {
